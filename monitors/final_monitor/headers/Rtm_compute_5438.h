...
init -> true
unique_id_counter -> 1
input_exp_ltxeq -> 'a \rightarrow (a \until_{<10} b)'
...
#ifndef RTM_COMPUTE_5438_H_
#define RTM_COMPUTE_5438_H_
...
template <class T>
three_valued_type _rtm_compute_5438_0(T& trace, timespan& t) {
  return [](T& trace, timespan& t) {
    auto x = [](T& trace, timespan& t) {
      auto x = prop<T>(trace, PROP_a, t);
      return b3_not(x);
    }(trace, t);
    auto y = until_less<T, Eval_until_less_3<T>, 10>(trace, t);
    return b3_or(x, y);
  }(trace, t);
};
